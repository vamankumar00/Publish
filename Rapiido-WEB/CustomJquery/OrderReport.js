...
$(document).ready(function () {
    ...
    $('#GenerateOrderReport').click(function () {
        ...
        Common.Ajax('POST', url + 'GetOrderReport', JSON.stringify(request), 'json', GetCurrentOrdersHandler);
    });
});
function GetCurrentOrdersHandler(response) {
    ...
    $.each(response, function (i, item) {
        ...
        row += '<tr><td class=OrderId style= "display:none">' + item.OrderId + '</td><td class="show-details"><i class="glyphicon glyphicon-plus"></i></td><td class=OrderNo>' + item.OrderNo + '</td><td>' + item.OrderDate + '</td><td>' + item.Fname + '</td><td>' + item.TelephoneNo + '</td><td>' + item.DriverName + '</td><td>' + item.DriverStatusDesc + '</td><td>' + item.OrderNetAmount + '</td>' +
    ...
    //    var MasterId = $(this).find('td.OrderId').text();
    //    if (MasterId != "") {
    //        var OrderMasterId = '{"Id":"' + MasterId + '"}';
