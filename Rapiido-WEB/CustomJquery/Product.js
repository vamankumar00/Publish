...
$(document).ready(function () {
    ...
    $('#AddDealProduct').click(function () {
        ...
        var ItemVeriationId = $('#ddlDealVarient').val();
        var ItemRateId = $('#ddlDealVarient').find(':selected').data('itemrateid');
        var Size = $('#ddlDealVarient').find(':selected').text();
        var QTY = $('#txtDealItemQty').val();
        var html = '<tr>' +
            '<td hidden class="Dl_CategoryId">' + CategoryId + '</td>' +
            '<td hidden class="Dl_SubCategoryId">' + SubCategoryId + '</td>' +
            '<td hidden class="Dl_ItemId">' + ItemId + '</td>' +
            '<td hidden class="Dl_ItemVeriationId">' + ItemVeriationId + '</td>' +
            '<td hidden class="Dl_ItemRateId">' + ItemRateId + '</td>' +
            '<td>' + CategoryName + '</td>' +
            '<td>' + ItemName + '</td>' +
            '<td>' + Size + '</td>' +
        ...
        $('#addDealDetailTable > tbody').prepend(html);
    });
    $('#SubmitDeal').click(function () {
        var requests = {};
        requests.DefaultClientId = DefaultClientId;
        requests.DealDetail = new Array();
        $("#addDealDetailTable TBODY tr").each(function () {
            var row = $(this);
            var Deal = {};
            Deal.CategoryId = row.find(".Dl_CategoryId").html();
            Deal.SubCategoryId = row.find(".Dl_SubCategoryId").html();
            Deal.ItemId = row.find(".Dl_ItemId").html();
            Deal.ItemVeriationId = row.find(".Dl_ItemVeriationId").html();
            Deal.ItemRateId = row.find(".Dl_ItemRateId").html();
            Deal.QTY = row.find(".Dl_ItemQTY").html();
            requests.DealDetail.push(Deal);
        });
        console.log(JSON.stringify(requests));
        Common.Ajax('POST', url + 'InsertDeal', JSON.stringify(requests), 'json', AddDealSaveHandler);
    });
});
